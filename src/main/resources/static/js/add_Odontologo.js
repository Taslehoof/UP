window.addEventListener('load', () => {
    const form = document.querySelector('#agregar_odontologo_form');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        // Armar payload (Para el OdontologoController)
        const payload = {
            nombre: document.querySelector('#add_nombre')?.value.trim() || '',
            apellido: document.querySelector('#add_apellido')?.value.trim() || '',
            matricula: document.querySelector('#add_matricula')?.value.trim() || ''
        };

        try {
            const resp = await fetch('/odontologo', {
                method: 'POST',
            ...
            if (!resp.ok) {
                let errorMsg = 'No se pudo agregar al Odontólogo. Intentá nuevamente.';
                try {
            ...
            if (data && data.id) {
                ...
                if (table) {
                    ...
                    // Creamos los mismos botones del get_Odontologo
                    let deleteButton = `<button
                      ...
                      data-odontologo-id="${data.id}"
                      data-odontologo-nombre="${data.nombre} ${data.apellido}">
                    ...
                    row.innerHTML =
                        ...
                        '<td class="td_matricula">' + (data.matricula ?? payload.matricula) + '</td>' +
                        '<td>' + updateButton + '</td>' +
                        '<td>' + deleteButton + '</td>';
                } else {
            ...
            showToast('Odontologo agregado', 'success');
        } catch (err) {
            console.error(err);
            showToast(err.message || 'No se pudo agregar el odontologo. Intentá nuevamente.', 'danger');
        }
    });
